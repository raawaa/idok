...
- **智能缓存**: 自动缓存扫描结果，大幅提升二次启动速度。缓存文件位置：`%APPDATA%\idok\database.json`
- **文件变化检测**: 智能检测文件变化，只在必要时重新扫描
...
- 在 release 页面找到最新的版本（标记为 **Latest**）
...
- 扫描完成后，您将看到所有媒体的封面展示
- 对于没有 NFO 文件的影片，idok 会从文件名中提取番号，并显示为空白影片信息卡片
